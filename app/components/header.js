    ...
    return (
        <header className="">
            <div className="p-4 hidden sm:flex gap-4 my-auto fixed z-50 w-full backdrop-blur-sm bg-white/5 border-b-[1px] border-white/10">
                <Link href={'/'}><Image alt="Logo" src="/images/logo.png" width={40} height={40} className="size-5 hover:size-7 m-1 hover:m-0 transition-all duration-300 ease-out rotate-[-45deg] hover:rotate-[0deg] active:size-5 active:m-1" /></Link>
            ...
            <div className="hidden max-sm:flex fixed z-50 w-full justify-end bottom-0 p-4">
                <button onClick={toggleNavBar} className={`bg-highlighter shadow-md rounded-full transition-all duration-300 p-1`}>
                    <Image src="/images/logo.png" alt="Logo" width={48} height={48} className={`transition-all duration-300 ease-out size-8 m-1 ${navBarOpened ? "rotate-[0deg]" : "rotate-[-45deg]"}`} />
                </button>
            </div>
        </header>
    );
